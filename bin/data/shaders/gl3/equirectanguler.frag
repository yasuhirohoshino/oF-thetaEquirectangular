...
vec4 getBlendedPixel(float u, float v, int witchCam, float thresholdRadius, vec3 sphere, float mixValue, float blending)
{
    ...
    if( witchCam == 0 ) {
        ...
        if ( dist < 0 ) {
            ...
            vec4 baseColor = texture(source, clamp(vec2(u, v), vec2(0.0), resolution));
            vec4 blendColor = texture(source, vec2(blendU, blendV));
        ...
        if ( dist < 0 ) {
            ...
            vec4 baseColor = texture(source, vec2(blendU, blendV));
            vec4 blendColor = texture(source, clamp(vec2(u, v), vec2(0.0), resolution));
...
void main (void) {
    ...
    if( sph_2.y >= 0.0 ) {
        ...
        fragColor = getBlendedPixel(u, v, 0, radius, sph_2, xTransition, blending);
    } else {
        ...
        float v = center.y + radius * sin(phi);
        float blending = clamp(blendingRatio, 0.0001, 0.99999);
        fragColor = getBlendedPixel(u, v, 1, radius, sph_2, xTransition, blending);
    }
}